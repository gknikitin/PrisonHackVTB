...
public class ConstraintLayout extends ViewGroup {
    ...
    public void onMeasure(int i, int i2) {
        ...
        boolean z2 = (mode == this.mLastMeasureWidthMode && mode2 == this.mLastMeasureHeightMode) ? USE_CONSTRAINTS_HELPER : false;
        ...
        if (size3 > 0) {
            boolean z4 = this.mLayoutWidget.getHorizontalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT ? USE_CONSTRAINTS_HELPER : false;
            boolean z5 = this.mLayoutWidget.getVerticalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT ? USE_CONSTRAINTS_HELPER : false;
            ...
            int i24 = 0;
            while (i23 < size3) {
                ConstraintWidget constraintWidget = this.mVariableDimensionsWidgets.get(i23);
                    ...
                    if (layoutParams.isHelper || layoutParams.isGuideline) {
                        ...
                        if (view.getVisibility() != 8 && (!z3 || !constraintWidget.getResolutionWidth().isResolved() || !constraintWidget.getResolutionHeight().isResolved())) {
                            if (layoutParams.width != -2 || !layoutParams.horizontalDimensionFixed) {
                                i10 = View.MeasureSpec.makeMeasureSpec(constraintWidget.getWidth(), CrashUtils.ErrorDialogData.SUPPRESSED);
                            } else {
                                i10 = getChildMeasureSpec(i15, paddingRight, layoutParams.width);
                            }
                            if (layoutParams.height != -2 || !layoutParams.verticalDimensionFixed) {
                                i11 = View.MeasureSpec.makeMeasureSpec(constraintWidget.getHeight(), CrashUtils.ErrorDialogData.SUPPRESSED);
                            } else {
                            ...
                            if (measuredWidth != constraintWidget.getWidth()) {
                                ...
                                if (z4 && constraintWidget.getRight() > i22) {
                                    i22 = Math.max(i22, constraintWidget.getRight() + constraintWidget.getAnchor(ConstraintAnchor.Type.RIGHT).getMargin());
                                }
                            ...
                            if (measuredHeight != constraintWidget.getHeight()) {
                                ...
                                if (z5) {
                                    ...
                                    if (constraintWidget.getBottom() > i14) {
                                        i13 = Math.max(i14, constraintWidget.getBottom() + constraintWidget.getAnchor(ConstraintAnchor.Type.BOTTOM).getMargin());
                                        i12 = i13;
                                        z6 = USE_CONSTRAINTS_HELPER;
                                    }
                                } else {
            ...
            for (int i31 = 0; i31 < i30; i31++) {
                ConstraintWidget constraintWidget2 = this.mVariableDimensionsWidgets.get(i31);
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            int i32 = resolveSizeAndState & ViewCompat.MEASURED_SIZE_MASK;
            int i33 = resolveSizeAndState2 & ViewCompat.MEASURED_SIZE_MASK;
            int min = Math.min(this.mMaxWidth, i32);
            int min2 = Math.min(this.mMaxHeight, i33);
